...
.. note:: 如果使用 PostgreSQL 或是 Interbase 的 PDO 驅動程式，這個函數會需要一個 $name 參數，用來指定適當的序列來檢查插入的 id 。
...
這個函數簡化了插入資料到資料庫的過程。它會回傳一個正確格式的 SQL 插入字串。例如：
...
這個函數簡化了撰寫資料庫更新的過程。它會回傳一個正確格式的 SQL 更新字串。例如：
